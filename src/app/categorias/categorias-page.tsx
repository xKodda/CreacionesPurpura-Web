...
import { ArrowRight, Package, Star, TrendingUp, Sparkles } from 'lucide-react';
...
export default function CategoriasPage() {
  ...
  const categoryStats = categories.map(category => {
    ...
    return {
      ...
      newProducts,
      onSaleProducts,
      featuredProduct: categoryProducts[0]
    };
  });
    ...
    <div className="min-h-screen bg-brand-fondo">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-brand-fondoSec border border-brand-detalle rounded-2xl p-6 shadow-lg">
            <div className="flex items-center">
              <Package className="h-8 w-8 text-brand-acento" />
              <div className="ml-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          ...
          <Link href="/productos" className="group bg-brand-fondoSec border border-brand-detalle rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col items-center justify-center py-12 cursor-pointer">
            <div className="flex flex-col items-center">
              <Package className="h-12 w-12 text-brand-acento mb-4" />
          ...
          {categoryStats.map((category) => (
            <Link key={category.id} href={`/categorias/${category.id}`} className="group bg-brand-fondoSec border border-brand-detalle rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden cursor-pointer">
              <div className="aspect-square bg-gradient-to-br from-brand-fondo to-brand-fondoSec flex items-center justify-center relative overflow-hidden">
                {category.featuredProduct ? (
                  <Image
                    src={category.featuredProduct.imageUrl || '/placeholder-product.jpg'}
                    alt={category.name}
                    width={100}
                    height={100}
                  ...
                  <div className="text-brand-acento text-4xl font-bold">
              ...
              <div className="p-6">
                ...
                <div className="space-y-2 mb-4">
                  ...
                  {category.newProducts > 0 && (
                    <p className="text-green-600 text-sm font-medium">
                      {category.newProducts} nuevos
                    </p>
                  )}
                  {category.onSaleProducts > 0 && (
                ...
                >
                  Explorar categoría <ArrowRight className="ml-2 h-4 w-4" />
                </span>
              </div>
            </Link>
          ))}
