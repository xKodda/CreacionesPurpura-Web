...
export default function ProductosPage() {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    const filtered = products.filter(product => {
      // Category filter
      if (filters.category !== 'all' && product.category?.id !== filters.category) {
        return false;
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="bg-brand-fondoSec rounded-2xl shadow-lg p-6 mb-8">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-brand-acento" />
            <input
              type="text"
              placeholder="Buscar productos..."
              value={filters.searchQuery}
              onChange={(e) => setFilters(prev => ({ ...prev, searchQuery: e.target.value }))}
              className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-acento focus:border-brand-acento text-brand-principal"
            />
          </div>
        </div>
        ...
        <div className="bg-brand-fondoSec border border-brand-detalle rounded-2xl shadow-lg p-6 mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div className="flex items-center space-x-4">
              <button
                ...
                className="flex items-center space-x-2 px-4 py-2 bg-brand-acento text-white rounded-lg hover:bg-brand-principal transition-colors shadow-sm"
              >
                <SlidersHorizontal className="h-4 w-4" />
                <span>Filtros</span>
                {activeFiltersCount > 0 && (
                  <span className="bg-white text-brand-acento rounded-full px-2 py-1 text-xs font-bold">
                    {activeFiltersCount}
                  </span>
                )}
              </button>
              ...
              {activeFiltersCount > 0 && (
                <button
                  ...
                  className="flex items-center space-x-1 text-brand-principal hover:text-brand-acento transition-colors"
                >
                  <X className="h-4 w-4" />
            ...
            <div className="flex items-center space-x-4">
              <select
                ...
                className="border-2 border-brand-acento rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-acento bg-white text-brand-principal shadow-sm"
              >
              ...
              <div className="flex border-2 border-brand-acento rounded-lg overflow-hidden">
                <button
                  ...
                  className={`p-2 ${viewMode === 'grid' ? 'bg-brand-acento text-white' : 'text-brand-principal hover:text-brand-acento'}`}
                >
                  <Grid className="h-4 w-4" />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-2 ${viewMode === 'list' ? 'bg-brand-acento text-white' : 'text-brand-principal hover:text-brand-acento'}`}
                >
          ...
          {/* Advanced Filters */}
          {showFilters && (
            <div className="mt-6 pt-6 border-t border-brand-detalle">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full border-2 border-brand-acento rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-acento bg-white text-brand-principal shadow-sm"
                  >
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <input
                      ...
                      className="w-full h-2 bg-brand-acento rounded-lg appearance-none cursor-pointer"
                    />
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center">
                      <input
                        ...
                        className="rounded border-brand-acento text-brand-acento focus:ring-brand-acento"
                      />
          ...
          <div className="text-center py-12">
            <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md mx-auto">
              ...
              <button
                ...
                className="px-4 py-2 bg-brand-acento text-white rounded-lg hover:bg-brand-principal transition-colors"
              >
                Reintentar
              </button>
            </div>
        ...
        {!loading && !error && filteredAndSortedProducts.length > 0 ? (
          <div className={viewMode === 'grid' 
            ...
            {filteredAndSortedProducts.map((product) => (
              <div key={product.id} className="bg-brand-fondoSec rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="text-center py-12">
            <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md mx-auto">
              <Search className="w-12 h-12 text-brand-acento mx-auto mb-4" />
