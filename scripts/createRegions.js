...
async function main() {
  console.log('🗺️ Iniciando creación de regiones y comunas...');
  ...
  for (const region of regiones) {
    try {
      ...
      console.log(`✅ Región creada: ${region.name}`);
      ...
      for (const comuna of region.comunas) {
        try {
          ...
          console.error(`❌ Error creando comuna ${comuna}:`, err.message);
        }
      }
      console.log(`  📍 ${region.comunas.length} comunas agregadas`);
    } catch (err) {
      console.error(`❌ Error creando región ${region.name}:`, err.message);
    }
  }

  console.log('🎉 Regiones y comunas creadas exitosamente!');
}

main()
  .catch((e) => {
    console.error('❌ Error durante la creación de regiones:', e);
    process.exit(1);
  })
  .finally(async () => {
